...
let transactions = [];
const API_URL = '/api/transactions'; // Vercel handles this automatically
...
async function loadData() {
    try {
        ...
        // Convert string ID to number if needed or ensure types
        transactions = data.map(t => ({
            ...t,
            amount: parseFloat(t.amount),
            id: String(t.id) // Use string for IDs from DB to be safe
        }));
...
if(form) form.addEventListener('submit', async (e) => {
    ...
    // Generate numeric ID for DB bigInt roughly
    const newTrx = {
        id: Date.now(), 
        title,
        amount: finalAmount,
    ...
    try {
        ...
        alert('Failed to save online. Check connection.');
        console.error(err);
    } finally {
...
navItems.forEach(item => {
    item.addEventListener('click', (e) => {
        ...
        // Re-render chart if switching to home (canvas issue fix)
        if(tabId === 'home' && cashflowChart) {
           cashflowChart.update(); 
        }
    });
});
...
// --- Initial Load ---
loadData(); // Fetch from Cloud
`;

fs.writeFileSync('main.js', content, 'utf8');
console.log('main.js updated for Online Sync');
